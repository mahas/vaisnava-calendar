...
* **Loading Feedback**: Displays a placeholder item (`"Buscando (despertando servidor si es necesario)..."`) inside the dropdown immediately upon typing to manage Render free-tier wakeup expectations.
...
* **Cache Invalidation**: On page load, a cache version flag (`GCAL_CACHE_VERSION = "v8"`) is checked. If it mismatches the stored version, old caches are automatically deleted to force loading the latest backend calculation updates.
...
* **Safari/iOS PWA Standalone Click Fix**: Programmatic `.click()` triggers on download anchors (like the `.ics` files generated by `descargarICSFile` and `exportarAGoogleCalendar`) are blocked by Safari webviews unless the anchor is attached to the document. To circumvent this, the anchor is temporarily appended to `document.body` and removed immediately after clicking.
...
2. **Semantic Biography Linking**:
   - Event descriptions are scanned for names of prominent acharyas, topics, and characters (e.g., *Srila Prabhupada*, *Sri Caitanya Mahaprabhu*, *Srimati Radharani*, *Krishna*, *Vamana*, *Varaha*, *Bhagavad-gita*).
   - If matched, a **"Ver en BhaktiLib" / "View on BhaktiLib"** button (`.bhaktilib-semantic-btn`) is rendered beside the event.
   - Links target the canonical author or topic permalinks on BhaktiLib (e.g. `https://bhaktilib.com/autor/srimati-radharani/`, `https://bhaktilib.com/autor/krishna/`, `https://bhaktilib.com/tema/bhagavad-gita/`).
